...
library(htmltools) # in dit script gebruikt om de "htmlEscape" bij de popup te zetten
  ...
  output$map <- renderLeaflet({
    ...
    #Indien je onderstaade in commentaar zet en addTiles()%>% uit commentaar, dan krijg je een kaart in kleur
     addProviderTiles(providers$Stamen.TonerLite ) %>%
      #addTiles()%>%
      addCircleMarkers(data = adreslocaties_subset(), radius = ~Aantal.artsen * 3, fillColor = "red" , color = "red", popup = ~htmlEscape(Praktijk)) 
  })
}
